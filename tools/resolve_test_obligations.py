...
TODO_TEST_LABELS: set[str] = {label for label, cmd in TEST_COMMANDS.items() if not cmd.strip()}
...
def is_packaging_file(path: Path) -> bool:
    name = path.name
    return name in PACKAGING_FILES or any(name.startswith(p) for p in PACKAGING_PREFIXES)
...
def classify_path(path: Path) -> FileObligations:
    ...
    if is_under(p, TOOLS_ROOT):
        result.add(Obligation(
            ...
            checks={"lint", "format", "typecheck", "tool-selfcheck", "agent-docs-consistency"},
            notes=[
    ...
    if is_under(p, SOURCE_ROOT):
        ...
        if not parts:
            result.add(Obligation(
                ...
                hard_stop=True,
                notes=["Pfad liegt unter SOURCE_ROOT ohne klassifizierbares Segment."],
            ))
            return result
        ...
        if "domain" in semantic_parts:
            ...
            ob.notes.append("Neuer Domain-Begriff erfordert Sprechakt SP1 oder Glossarentscheidung.")
            ob.notes.append("TODO: 'domain-tests' durch projektspezifischen Befehl ersetzen.")
            result.add(ob)
            return result
        ...
        if "system" in semantic_parts:
            ...
            ob.notes.append("TODO: 'system-tests' durch projektspezifischen Befehl ersetzen.")
            result.add(ob)
            return result
        ...
        if "infrastructure" in semantic_parts:
            ...
            ob.notes.append("TODO: 'infrastructure-tests' durch projektspezifischen Befehl ersetzen.")
            result.add(ob)
            return result
        ...
        if "adapters" in semantic_parts:
            ...
            ob.notes.append("TODO: 'adapter-tests' durch projektspezifischen Befehl ersetzen.")
            result.add(ob)
            return result
        ...
        if "cli" in semantic_parts or "entrypoints" in semantic_parts:
            ...
            ob.notes.append("TODO: 'cli-tests' durch projektspezifischen Befehl ersetzen.")
            result.add(ob)
            return result
        ...
        if "shared" in semantic_parts:
            ...
            ob.notes.append("Import-Checker schlägt an, wenn shared projektinterne Räume importiert.")
            ob.notes.append("TODO: 'affected-product-tests' durch projektspezifischen Befehl ersetzen.")
            result.add(ob)
            return result
            ...
            if obligation.checks:
                ...
                for check in sorted(obligation.checks):
                    ...
                    if cmd:
                        ...
                        print(f"  - {check}: [KEIN BEFEHL KONFIGURIERT — bitte eintragen]")
            ...
            if obligation.tests:
                ...
                for test in sorted(obligation.tests):
                    ...
                    if cmd:
                        ...
                        print(f"  - {test} [TODO: projektspezifischen Befehl in TEST_COMMANDS eintragen]")
...
def selfcheck(strict: bool = False) -> int:
    ...
    values = [
        str(SOURCE_ROOT), str(TEST_ROOT), str(DOCS_ROOT), str(TOOLS_ROOT),
        *STANDARD_CHECKS.values(),
        PYTHON_TEST, FULL_VALIDATION, IMPORT_LAYER_CHECK,
    ]
    ...
    if strict:
        ...
        if open_labels:
            print(f"Selfcheck (instantiated): FAILED — "
            ...
            print("einen echten Befehl in TEST_COMMANDS haben (z.B. 'pytest tests/domain').")
            return 1
    ...
    print("  E1 — nach Session mit Plan/MITTEL/Sprechakt/Task-Schnitt/API/Dokumentdrift")
